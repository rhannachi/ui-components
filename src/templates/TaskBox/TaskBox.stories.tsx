  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('👇 Get the first Task "task-1" ', async () => {
      ...
      await step('👇 Search for the "button-icon-task" button', async () => {
        ...
        await step(
          '👇 Click on the Pin icon and verify that it changes color from "cyan-400" to "gray-200"',
          async () => {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('👇 Get the first Task "task-1" ', async () => {
      ...
      await step('👇 Search for the "button-icon-task" button', async () => {
        ...
        await step(
          '👇 Click on the Pin icon and verify that it changes color from "gray-200" to "cyan-400"',
          async () => {
            const icon: HTMLElement = await findByRole(button, 'icon-task')
            await expect(icon.classList).toContain('fill-gray-200')
            await userEvent.click(button, { detail: 100 })
...
export const DisabledEditTask: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('👇 Get the first Task "task-1" ', async () => {
      ...
      await step(
        '👇 Search for the input text field "input-task" and attempt to make modifications',
        async () => {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('👇 Get the first Task', async () => {
      ...
      await step('👇 Search for the checkbox and verify that it is not checked', async () => {
        ...
        await step(
          '👇 Search for the input text and verify that it contains the correct color "gray-500" and is not crossed out',
          async () => {
            ...
            await step(
              '👇 Click on the checkbox, verify that the text changes to the "gray-200" color, becomes crossed out, and the button-icon-task is no longer displayed',
              async () => {
