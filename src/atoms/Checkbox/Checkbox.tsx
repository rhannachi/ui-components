...
const Checkbox = ({ checked = false, size = 'base', color = 'cyan', label, onChange, className, ...other }: CheckboxProps) => {
  ...
  return (
    <div className={clsx(className, 'flex items-center')}>
        <input {...other}
          ...
          <div className={clsx(COLORS_BOX[color] , SIZE_BOX[size],"bg-white border-2 rounded flex flex-shrink-0 justify-center items-center")}>
            <svg className={clsx(SIZE_SVG[size] , "fill-current hidden  pointer-events-none")} version="1.1"
              ...
              <g fill="none" fill-rule="evenodd">
                <g transform="translate(-9 -11)" className={clsx(COLORS_SVG[color])} fill-rule="nonzero">
                  <path
